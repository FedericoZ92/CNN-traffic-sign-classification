...
save_weights = 1
simulation_name = 'TESTEMP'#'43cl._1ite_segm'#'std_43cl_10ite_32x32_halveall_fcl256' #To be changed every time architecture or data changes to save the weights. 
...
def load_dataset(png_needed):    
    ...
    def load_test_images(png_needed):
        ...
        for classes in range(0,num_of_classes):          
           ...
           j = 0    
           for file in glob.glob(homedir + str(classes) + '/*.png'):    
               if j >= i * cross_fold / 3 and j <= i * (cross_fold + 1)/ 3:
                  im_tempi = Image.open(file)
