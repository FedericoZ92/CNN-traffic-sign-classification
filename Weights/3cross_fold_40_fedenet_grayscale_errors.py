...
#GLOBAL SETTINGS
homedir = '/home/fede/Desktop/cross_fold_GTSRB/'#/home/fede/Desktop/Full_set/' #Main directory of dataset
...
num_epochs_ = 2 #Times all images are forward-propagated.
batch_size = 128
image_size = 32
...
learning_rate_ = 0.01 #Kepp between 0.001 and 0.020
momentum_ = 0.9 #Keep close to 0.9
toLoadWeights = True #To resume training from previous state.
save_weights = 1
simulation_name = '43cl._2ite_gray'#'43cl._1ite_segm'#'std_43cl_10ite_32x32_halveall_fcl256' #To be changed every time architecture or data changes to save the weights. 
show_features = 0 #1 to display features at layer_n_feats_arr layers, 0 not to.
layer_n_feats_arr = [1,2,3,4,5,6,8]#1,2,3,4,5,6,8]
show_examples = 1 #1 to classify sample images, 0 not to.
...
def prep_image_for_test_B(img, for_test): 
    ...
    im2 = zeros((1,crop_size,crop_size),int)
        
    #HERE im VALUES ARE BETWEEN 0 AND 1
    for i in range (0,crop_size):
       for j in range (0,crop_size):                      
               im2[0,i,j] = 255*255* (0.2990*im[i,j,0] + 0.5870*im[i,j,1] + 0.1440*im[i,j,2])
               #currval = float32(255*255*(0.2990*im[i,j,0] + 0.5870*im[i,j,1] + 0.1440*im[i,j,2]))              
               #im[i,j,0] = 255*255*(0.2990*im[i,j,0] + 0.5870*im[i,j,1] + 0.1440*im[i,j,2]) #255*255*im[i,j,0]
               #im[i,j,1] = 0#255*255*im[i,j,1]
               #im[i,j,2] = 0#255*255*im[i,j,2]
...
def load_dataset(png_needed):    
   ...
   def load_train_images(png_needed):
        ...
        for classes in range(0,num_of_classes): 
           ...
           if png_needed == 1:          
             for file in glob.glob(homedir + str(classes) + '/*.ppm'):# + str(classes) + '_train/*.ppm'):
             ...
             for file in glob.glob(homedir + str(classes) + '/*.png'):
                 im_temp = Image.open(file)
           ...
           j = 0    
           for file in glob.glob(homedir + str(classes) + '/*.png'):    
               if j <= i * cross_fold / 3 or j >= i * (cross_fold + 1)/ 3:
                  im_tempi = Image.open(file)
    ...
    def load_test_images(png_needed):
        ...
        for classes in range(0,num_of_classes):          
           if png_needed == 1:
             for file in glob.glob(homedir + str(classes) + '/*.ppm'):
                 im_temp = Image.open(file)                 
                 file = file[:-4]
                 im_temp.save(file + '.png')
             for file in glob.glob(homedir + str(classes) + '/*.png'):
           ...
           j = 0    
           for file in glob.glob(homedir + str(classes) + '/*.png'):    
               if j >= i * cross_fold / 3 && j <= i * (cross_fold + 1)/ 3:
                  im_tempi = Image.open(file)
...
def build_cnn2(input_var=None):
    ...
    #0
    network[0] = lasagne.layers.InputLayer(shape=(None, 1, crop_size, crop_size),
                                        input_var=input_var)   
    #1
    ...
    #10
    network[9] = DropoutLayer(network[8], p=0.25)
    ...
    #12          
    network[11] = DropoutLayer(network[10], p=0.25)          
...
def build_cnn3(input_var=None):
    ...
    #
    network[10] = DropoutLayer(network[9], p=0.25)
    ...
    #          
    network[12] = DropoutLayer(network[11], p=0.25)          
