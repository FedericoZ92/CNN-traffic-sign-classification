...
#GLOBAL SETTINGS
homedir = '/home/fede/Desktop/Full_set1/'#/home/fede/Desktop/Full_set/' #Main directory of dataset
...
save_weights = 1
simulation_name = '43cl._21ite_gray'#'43cl._1ite_segm'#'std_43cl_10ite_32x32_halveall_fcl256' #To be changed every time architecture or data changes to save the weights. 
...
############################################################################## 
    ...
    if show_confusion_matrix == 1:
       ...
       for i in range(0,25):#len(X_test)):#len(X_test)):  #(start,end,step)
           ...
           ##decision = np.argmax(arr2)#arr2.argmax(axis = 1)#index(max(arr2))#unravel_index(net_output.argmax(), net_output.shape())
           ##print(decision)
           maxv = np.float64(0)
           decision = 0
           arr2.astype(float64)
    ...
    if show_features == 1:
       ...
       for file in glob.glob(homedir + "*.png"):
           for i in range(0,len(layer_n_feats_arr)):
               ...
               fig, ax = plt.subplots(ratio*3, ncols=8, sharex='col', sharey='row', figsize = (8,ratio)) #sharex=True, sharey=False
